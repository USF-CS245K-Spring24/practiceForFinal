...
public class HashTableLinearProbing {
    ...
    public void put(int key, Object value) {
        ...
        while (table[i] != null && table[i].getKey() != key) {
            i = (i + 1) % maxSize;
            // Went a full circle, the table is full but no key
            if (i == hashIndex) {
                throw new IllegalArgumentException();//no place to insert
            }
        }
        if (table[i] == null)
            table[i] = new HashEntry(key, value); // found a place to insert this key, value
        else { // found the key, just need to change the value
